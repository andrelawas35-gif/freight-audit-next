...
import {
  ...
  CarrierMark, RuleTag, TableFooter, ConsoleErrorState,
} from '@/components/ui/primitives';

export const dynamic = 'force-dynamic';
const CARRIER_DISPLAY_LIMIT = 50;
...
// ── Normalize rule names from select values ────────────
...
export default async function CarriersPage() {
  ...
  if (loadError) {
    return (
      <div style={{ padding: 14, maxWidth: 1340, margin: '0 auto' }}>
        <ConsoleErrorState
          heading="Couldn't load carriers"
          message={loadError}
            ...
            {scorecards.length === 0 && (
              <tr>
                <td colSpan={8} style={{ textAlign: 'center', color: 'var(--ink-faint)', padding: 30 }}>
                  No carrier data yet. Add carrier records, then run the audit scripts.
                </td>
              </tr>
            )}
          </tbody>
        </table>
        <TableFooter showing={scorecards.length} total={scorecards.length} label="carriers" hasMore={hasMoreCarriers} />
      </Card>
