...
const DISPUTE_DISPLAY_LIMIT = 200;

// Extend the base Dispute type to include Lookup fields from joins
 ...
 try {
    const [disputesRaw, clientsRaw, auditResultsRaw] = await Promise.all([
      fetchRecords('Disputes', {
        ...
        maxRecords: DISPUTE_DISPLAY_LIMIT + 1,
      }),
    ...
    // Apply strict types
    hasMoreRows = disputesRaw.length > DISPUTE_DISPLAY_LIMIT;
    const disputes = (disputesRaw as FetchedDispute[]).slice(0, DISPUTE_DISPLAY_LIMIT);
      ...
      <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 1340, margin: '0 auto', width: '100%' }}>
       ...
       {/* KPI Row */}
        ...
        <div>
          ...
          <DisputesView initialRows={rows} loadError={loadError} hasMoreRows={hasMoreRows} />
        </div>
