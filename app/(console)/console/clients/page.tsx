...
const CLIENT_DISPLAY_LIMIT = 100;
...
export default async function ClientsPage() {
  ...
  let disputesRaw: any[] = [];
  let hasMoreClients = false;
  let loadError: string | null = null;

  try {
    const [clientsData, disputesData] = await Promise.all([
      fetchRecords('Clients', { maxRecords: CLIENT_DISPLAY_LIMIT + 1 }),
      fetchRecords('Disputes', { maxRecords: 1000 }),
    ]);

    hasMoreClients = clientsData.length > CLIENT_DISPLAY_LIMIT;
    const clientsRaw = (clientsData as any[]).slice(0, CLIENT_DISPLAY_LIMIT);
    ...
    scorecards = (clientsRaw as any[]).map(c => {
      ...
      const cDisputes = (disputesRaw as any[]).filter(d => {
        ...
        // Handle linked records (arrays from JOINs)
        return dClient === id;
      });
  ...
  return <ClientsView scorecards={scorecards} totals={totals} disputes={disputesRaw as any[]} hasMoreClients={hasMoreClients} />
