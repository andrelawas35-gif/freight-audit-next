...
import { SectionLabel, TableFooter, ConsoleErrorState } from '@/components/ui/primitives';
...
export default async function EnginePage() {
  ...
  try {
    const [clientsRaw, runsRaw, statsRaw] = await Promise.all([
      ...
      listRuns(RUN_HISTORY_LIMIT + 1),
      getRuleOutcomeStats(),
    ]);
    clients = (clientsRaw as Client[])
      .map((c) => ({ id: c.id, name: c['Company name'] || c.id }))
      .sort((a, b) => a.name.localeCompare(b.name));
    hasMoreRuns = runsRaw.length > RUN_HISTORY_LIMIT;
    runs = runsRaw.slice(0, RUN_HISTORY_LIMIT);
    ruleStats = statsRaw;
  } catch (err) {
  ...
  if (loadError) {
    return (
      <div style={{ padding: 14, maxWidth: 1100, margin: '0 auto' }}>
        <ConsoleErrorState
          heading="Couldn't load audit engine"
          message={loadError}
    ...
    <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
          ...
          {runs.length > 0 && (
            <TableFooter showing={runs.length} total={runs.length} label="runs" hasMore={hasMoreRuns} />
          )}
        </div>
      </div>
