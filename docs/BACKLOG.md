...
- [x] **(verify, codebase-wide) `sql.query('BEGIN'/'COMMIT')` atomicity on the HTTP `neon()` driver.** **DECIDED (2026-06-27).** Grilling decision: migrate ALL financial write paths to `sql.transaction([...])` (documented Neon contract). Order: `batchCreate` → `defineClauseAction` → `engine.ts`/`3pl-engine.ts` → `policy-service.ts`. Add integration test verifying rollback on partial failure. Do not rely on undocumented raw `BEGIN`/`COMMIT` behavior.
  - **DONE (2026-07-10).** All raw `BEGIN`/`COMMIT`/`ROLLBACK` eliminated from the codebase:
    - `lib/db/records.ts` `batchCreate()` — `sql.transaction((txn) => [...])` replaces raw BEGIN/COMMIT.
    - `lib/audit/engine.ts` — `sql.transaction((txn) => [...])` bundles all INSERT + UPDATE queries atomically.
    - `lib/audit/3pl-engine.ts` — `sql.transaction((txn) => [...])` bundles findings INSERT + mark-audited UPDATE atomically.
    - `lib/ingestion/3pl/stage.ts` — `sql.transaction((txn) => [...])` for both `stageFulfillment` and `stageStorage`.
    - `lib/intelligence/policy-service.ts` — `sql.transaction((txn) => [...])` for backtest run + results + completion UPDATE. Run ID pre-generated via `crypto.randomUUID()` outside the transaction.
    - Zero raw `sql.query('BEGIN')` calls remain in the codebase.
...
- [x] **Wire the client path through `getTenantSql` (ADR 0013).** Add an optional `db` param to the `records.ts` read helpers (default `getSql()`); portal data-loader acquires one `getTenantSql(session.user.clientId)` per request and releases it in `finally`. Staff console / audit engine / BI stay on owner `getSql()`.
  - **DONE (2026-07-10).** `getPortalDashboardData()` acquires `getTenantSql(clientId)` with `db.release()` in `finally`. `fetchRecords`, `fetchRecord`, `findByField` accept optional `db` param. Staff paths use default `getSql()`. See `lib/portal/data-loader.ts`, `lib/db/records.ts`.
- [x] **Extend the restricted role to the portal read-set + RLS rollout migration (ADR 0013 D3/D5).** Grant SELECT + add RLS policies for `Clients` (own-row `id = app.current_tenant`), `policy_rulesets`, `policy_attestations`, `policy_scope_exclusions`.
  - **DONE (2026-06-27).** Migration 0018 (`0018_rls_rollout_portal_read_set.sql`) applied. Grants SELECT on portal read-set tables; ENABLE ROW LEVEL SECURITY + CREATE POLICY for each; re-asserts policies on Phase-1 business tables. FORCE RLS applied only on Phase-1 tables; portal tables use RLS without FORCE.
- [x] **Behavioral RLS isolation test, gated on `TEST_DATABASE_URL` (ADR 0013 D4).** Connect as `app_tenant`, assert 0 rows with no tenant set, seed tenant A/B, assert A cannot read B.
  - **DONE (2026-06-27).** `lib/__tests__/rls-isolation.test.ts` — static policy lint (always runs) + behavioral integration test (gated on `TEST_DATABASE_URL`). Tests: no-tenant-context returns 0 rows; cross-tenant read isolation; cross-tenant write rejection.
- [x] **`findOrCreateNextDraft(clientId)` + copy-forward; fix `defineClauseAction` (ADR 0014).** Client Define attaches the rule to a single designated per-client draft ruleset.
  - **DONE (2026-06-27).** Shipped in Wave 2. `defineClauseAction` uses `sql.transaction()` for atomic scope-exclusion UPDATE + rule INSERT.
- [x] **Staff correctness gate for client-defined rules (ADR 0015).** Add `policy_rules.staff_reviewed boolean NOT NULL DEFAULT false`.
  - **DONE (2026-06-27).** Migration 0019 (`0019_staff_reviewed_column.sql`) shipped. `staff_reviewed` column + staff review queue in console.

**Phase 2 Build Plan — Remaining Tasks (BUILD-PLAN-PHASE2.md, 2026-07-10)**

- [x] **E7: Data Maturity Audit ($500 deliverable).** `getDataReadinessReport(clientId)` in `lib/intelligence/reports.ts`; portal panel `data-readiness-panel.tsx`; staff console page `app/(console)/console/data-readiness/[clientId]/page.tsx`; wired into `ComplianceData` + `fetchComplianceData`.
  - **DONE (2026-07-10).** All four pieces shipped. Panel renders in portal Compliance tab below Gateway Readiness. Staff page shows per-field completeness, assessment tier, and pricing qualification.
- [x] **SG5: Gateway clean service (workspaces).** `services/gateway/` is now a self-contained npm workspace with zero cross-package imports. Own `db.ts` (neon connection), own `evaluator-types.ts` (shared type stubs). Compiles independently (`cd services/gateway && npm install && npx tsc --noEmit`). The Fastify service remains shelved (ADR 0016); the active gateway runs in-process via `app/api/v1/precheck/route.ts`.
  - **DONE (2026-07-10).** Root `package.json` includes `"workspaces": ["services/*"]`. Gateway compiles clean.
- [x] **SO1: Per-client-key auth to `/api/v1/precheck` (ADR 0016 D1).** `resolveClientIdFromApiKey()` scans `GATEWAY_API_KEY_<clientId>` env vars; primary auth strategy in `route.ts`. Falls back to single `GATEWAY_API_KEY` (legacy compat) and staff session. Client ID mismatch detection rejects body `clientId` that disagrees with the key-derived identity.
  - **DONE (2026-06-27).** Already implemented in `app/api/v1/precheck/route.ts`.
...
- [x] **(review 2026-06-26, live bug) Soft-delete filter crashes reads on tables without `deleted_at`.** `fetchRecords`/`fetchAllRecords` append `"<table>"."deleted_at" IS NULL` for **every** `TableName` ([`lib/db/records.ts:170`](../lib/db/records.ts), [`:220`](../lib/db/records.ts)), but migration `0008` only added the column to 11 of ~30 union members. Live: [`app/(console)/console/carriers/page.tsx:71`](<../app/(console)/console/carriers/page.tsx>) reads `'Carrier Codes'` (no `deleted_at`) → `column does not exist` → the carriers page falls into `ConsoleErrorState`. The other 16 columnless tables (`Invoice Lines`, `audit_jobs`, `gateway_decisions`, `ingestion_*`, `policy_backtest_*`, etc.) crash the same way when first read via these helpers.
  - **DONE (2026-06-27).** `SOFT_DELETE_TABLES: ReadonlySet<TableName>` gates the `deleted_at IS NULL` predicate — only tables that actually have the column get the filter. `fetchRecords('Carrier Codes')` works again. 16 tables covered (Invoices, Shipments, Audit Results, Disputes, Clients, Carriers, rulebook, client_policies, policy_documents, policy_rulesets, policy_rules, policy_scope_exclusions, policy_taxonomy_candidates, ingestion_exceptions, ingestion_batches, ingestion_records).
- [x] **(review 2026-06-26) Soft-delete bypassed by id/link reads.** `fetchRecord`, `fetchRecordsByIds`, and `fetchRecordsByLinkedIds` ([`lib/db/records.ts:237`](../lib/db/records.ts), [`:261`](../lib/db/records.ts), [`:285`](../lib/db/records.ts)) omit the `deleted_at` filter, so a soft-deleted row still surfaces when resolved by id or through a linked-record array (e.g. a tombstoned invoice still appears as a dispute's linked invoice).
  - **DONE (2026-06-27).** All id/link resolvers now gate `deleted_at IS NULL` via `SOFT_DELETE_TABLES.has()`. `fetchRecord`, `fetchRecordsByIds`, `fetchRecordsByLinkedIds`, `findByField`, and `fetchAllRecords` all apply the gated filter.
